...
class KoronaSpider(ShowingSpider):
    ...
    def parse_normal_showing(self, response, result_list):
        ...
        request = response.follow(url, callback=self.parse)
        self.set_next_func(request, self.parse_showing_seat_json)
        result = init_show_booking_loader(
            response=response, item=response.meta["dict_proto"])
        result_item = result.load_item()
        # set total_seat_count as it's needed to calculate booked_seat_count
        # latter
        result_item['showing']['total_seat_count'] = total_seat_count
        dict_proto = ShowingBookingLoader.to_dict(result_item)
        request.meta["dict_proto"] = dict_proto
        result_list.append(request)
    ...
    def parse_showing_seat_json(self, response, result_list):
        self._logger.debug("{} parse_showing_seat_json".format(self.name))
        try:
